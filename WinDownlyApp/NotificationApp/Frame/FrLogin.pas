...
begin
  if cbTermsCon.IsChecked = False then
    fnShowE('Check Terms & Conditions', Err)
  else
    fnReg;
end;
...
begin
  ...
  begin
    fnShowE('Username field empty', Err);
    Exit;
  end;

  if edLPass.Text = '' then
  begin
    fnShowE('Password field empty', Err);
    Exit;
  end;
...
begin
  ...
  begin
    if edRPass.Text <> edRRePass.Text then
      fnShowE('Passwords are not equal', Err)
    else
    begin
      if (edRUser.Text = '') or (edREmail.Text = '') or (edRPass.Text = '') then
      begin
        fnShowE('Fields cannot be empty', Err);
        Exit;
      end;
