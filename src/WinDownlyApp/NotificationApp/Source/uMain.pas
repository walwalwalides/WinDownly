...
procedure fnDefICO(btn, btnMV: TCornerButton; no: Integer;
  stat: Boolean = True);
...
begin
  try
    if (goFrame = Loading) or (goFrame = Login) then
    begin
      if (FMain.header.Visible = True) or (FMain.footer.Visible = True) then
        fnShowFootHead(False)
    end
    else if (goFrame <> Loading) or (goFrame <> Login) then
    begin
