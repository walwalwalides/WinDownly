...
const
  ...
  MSG_DATA_FOLDER: String = 'serData';
...
type
  ...
  Fstream: TStream);
    procedure SendFile(const from_name, to_name: string; Fstream: TStream);

    function GetData(const uname: string; lastcheck: TDateTime;
      var thischeck: TDateTime): TRtcArray;
  end;
...
begin
  ...
  if Copy(UserDataFileName, length(UserDataFileName), 1) <> FOLDER_DELIMITER
  then
...
begin
  ...
  try
    ...
    if not Write_File(UserListFileName, s) then
      raise Exception.Create('Error writing UserList to "' +
        UserListFileName + '".');
  finally
    userCS.Release;
  end;
          ...
          begin
            ...
            try
              if info2.isType['friends'] = rtc_Record then
                if not info2.asRecord['friends'].isNull[uname] then
                // we are in our friend's list
                begin
                  userCS.Acquire;
                  try
...
function TRtcMessengerUsers.isLoggedIn(uname: string;
  sessid: RtcString): boolean;
begin
  userCS.Acquire;
  try
    Result := False;
...
procedure TRtcMessengerUsers.SendData(const to_name: string;
...
begin
  ...
  try
    ...
    Write_File(fnamedb, s, File_Size(fnamedb));
  finally
    msgCS.Release;
  end;
...
function TRtcMessengerUsers.GetData(const uname: string; lastcheck: TDateTime;
  var thischeck: TDateTime): TRtcArray;
var
  fname: String;
  s, code: RtcString;
...
procedure TRtcMessengerUsers.SendFile(const from_name, to_name: string;
  Fstream: TStream);
var
  rec: tRtcRecord;
