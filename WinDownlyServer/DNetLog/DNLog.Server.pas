...
uses
  ...
  IdSocketHandle, System.SysUtils, System.classes;

{$DEFINE LOG_SERVER_AUTO_ON}

type
  TOnLogReceived = procedure(Sender: TObject; const ClientIP: string;
    const LogMessage: TDNLogMessage) of object;

type
  TDNLogServer = class(TObject)
  private
  ...
  protected
    procedure _OnUDPRead(AThread: TIdUDPListenerThread; const AData: TIdBytes;
  ...
  public
    ...
    property OnLogReceived: TOnLogReceived read FOnLogReceived
      write FOnLogReceived;
  end;
...
begin
  ...
  sockh.SetBinding(SERVER_BIND_ADDRESS_4, SERVER_BIND_PORT,
    TIdIPVersion.Id_IPv4);
  sockh := FServer.Bindings.Add;
  sockh.SetBinding(SERVER_BIND_ADDRESS_6, SERVER_BIND_PORT,
    TIdIPVersion.Id_IPv6);
...
begin
  ...
  Result.LogTimestamp := (ABytes[1] shl 24) + (ABytes[2] shl 16) +
    (ABytes[3] shl 8) + ABytes[4];
  ...
  if Assigned(FOnLogReceived) then
    try
      ...
      try
        ...
        // s := 'Replied from ' + FServer.Name + ' to "' + DataStringStream.DataString + '"';
